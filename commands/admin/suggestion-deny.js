    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('update-method')
                .setDescription('Whether or not deny update should be sent on seperate embed')
                .setRequired(true)
                .addChoices(
                    { name: 'Make_Seperate_Embed', value: 'update' },
                    { name: 'Update_On_Original', value: 'no update' },
                ))
        .addStringOption(option => option.setName('reason').setDescription('The reason for deny (optional)')),
    // .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
    async execute(interaction) {

        if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) return await interaction.reply({ content: "you must have admin to deny suggestions", ephemeral: true });
        ...
        await interaction.editReply({ content: `Successfully denied the suggestion.` });
